...
**Last synced:** 2026-05-29 (after `SOPHEX-DEMO-STORY-003` guided demo states closeout)
...
| `/property/:id`       | `prototype/src/pages/PropertyPage.tsx` | `mock-data` from `lib/runtime/public-property.ts`; primary **Underwrite in Studio** CTA → deal intake via `lib/workflow-identity`; guided demo rail + `?state=` handoffs to comps/report | `sandbox-api`, `provider`, `billing-auth` | Wave 6 cross-entity entry / `SOPHEX-DEMO-STORY-003` |
| `/property/:id/comps` | `prototype/src/pages/CompsPage.tsx`    | `mock-data` comps + blocked labels; guided demo rail + `?state=` handoff to report                                                                                                                                                                        | `provider`, `sandbox-api`                 | Wave 4 readiness rail / `SOPHEX-DEMO-STORY-003` |
...
| `/report/:id`         | `prototype/src/pages/ReportPage.tsx`   | `mock-data` sections; deterministic trust fixture states (`clean`, `blocked`, `low-evidence`, `provider-restricted`, `ready-for-review`); disabled export CTA plus export-gate handoff; guided demo rail; `simulated-cta` section review                                                    | `sandbox-api`, `hitl-legal`               | Wave 2/4 / `SOPHEX-REPORT-002` / `SOPHEX-OVERNIGHT-REPORT-MATRIX-001` / `SOPHEX-DEMO-STORY-003` |
| `/export/:id`         | `prototype/src/pages/ExportPage.tsx`   | `disabled-gate` generate; deterministic review-readiness fixture states (`clean`, `blocked`, `low-evidence`, `provider-restricted`, `ready-for-review`); consent/source-rights/reviewer/section gates; report/review/studio cross-links; guided demo rail; `MockBoundaryBanner` (`export` variant) | `hitl-legal`, `schema-db`                 | Wave 2/4/6 / Wave 26 / Wave 31 / `SOPHEX-EXPORT-GATE-001` / `SOPHEX-DEMO-STORY-003`             |
| `/review/:id`         | `prototype/src/pages/ReviewPage.tsx`   | `mock-ui` source gap cockpit; deterministic fixture states via `public-export-fixtures.ts`; source gap register; local-only reviewer actions; disabled export CTA; guided demo rail; report/export/source handoffs | `hitl-legal`, `sandbox-api`               | `SOPHEX-REVIEW-QUEUE-001` / `SOPHEX-DEMO-STORY-003`                                             |
| `/sources/:id`        | `prototype/src/pages/ReviewPage.tsx` (sources mode) | `mock-ui` source pack / citation drilldown; shared fixture states via `public-export-fixtures.ts`; source-rights blocker copy; local-only citation actions; disabled export CTA; guided demo rail; report/export/review handoffs | `hitl-legal`, `sandbox-api`, `provider`   | `SOPHEX-SOURCE-EVIDENCE-002` / `SOPHEX-DEMO-STORY-003`                                          |
...
**Guided demo layer (`SOPHEX-DEMO-STORY-003`):** `prototype/src/components/demo/GuidedDemoRail.tsx` + `appendExportFixtureStateQuery` in `public-export-fixtures.ts` render a mock-only guided rail on `/property/:id`, `/property/:id/comps`, `/report/:id`, `/export/:id`, `/review/:id`, and `/sources/:id`. Fixture state continuity is URL/query-state only (`?state=clean|blocked|low-evidence|provider-restricted|ready-for-review`); no persistence, sessions, or runtime API writes. Export remains disabled/gated in every state. Runtime, provider, billing, and CRE-bridge lanes remain future-gated.
