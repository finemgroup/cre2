...
| `/review/:id`         | `prototype/src/pages/ReviewPage.tsx`   | `mock-ui` source gap cockpit; deterministic fixture states (`clean`, `blocked`, `low-evidence`, `provider-restricted`, `ready-for-review`); source gap register; local-only reviewer actions (`mark gap reviewed`, expand detail, copy note); disabled export CTA; report/export handoffs; `runtimeServices.public.getReviewQueueView()` with fixture fallback | `hitl-legal`, `sandbox-api`               | `SOPHEX-REVIEW-QUEUE-001`                                             |
