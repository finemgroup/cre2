...
## Wave 8 Utilization Update

Wave 8 used CRE Platform cockpit doctrine without importing CRE runtime code:

- `SOPHEX-FE-UW-DEAL-COCKPIT-NEXT-ACTION` is extended by `DealCockpitPanel`, bento tiles, and the unified next-action adapter.
- `SOPHEX-FE-UW-GATE-RESOLUTION-COPY` is reinforced by confidence/escalation copy and shared HITL assignments.
- `SOPHEX-FE-UW-NAV-GROUPING` is extended by `DataWorkbenchShell` table/list/grid views for evidence review and source trace.
- `SOPHEX-FE-UW-ADVANCED-SURFACE-TRIGGERS` remains partially open for deeper route-by-route contextual promotion, but analyst review and AI task pulse now surface from the cockpit.
