...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `/` | `prototype/src/pages/LandingPage.tsx` | Public marketplace | A- | B | Public shell has forest chrome, presentation mode, continuity banner, visual baselines. | Strengthen proof blocks and conversion hierarchy if marketing becomes a priority. | Keep fixture/sandbox public search until data-source contracts and analytics redaction clear. | Mostly done |
| `/property/:id` | `prototype/src/pages/PropertyPage.tsx` | Public marketplace | A- | B | Authority labels, map HUD, evidence drawer, Studio handoff, visual snapshot. | Tune evidence drawer density and mobile map details. | Server-side field resolution, permissioned evidence, spatial source-rights. | Partial |
| `/property/:id/comps` | `prototype/src/pages/CompsPage.tsx` | Public marketplace | A- | B | Dense table, map/list context, blocked/private labels, visual snapshot. | Add saved-view or filter-empty polish if comps workflows expand. | Provider rights, comp source restrictions, private-value isolation. | Partial |
...
| `/report/:id` | `prototype/src/pages/ReportPage.tsx` | Public report | A- | B | Readiness rail, report posture, public/studio continuity, visual snapshot. | Strengthen report section presentation for advisor-style sharing. | Permission-filtered source bundles and section review state. | Partial |
...
| Route | File | Classification | Visual score | Runtime score | Current posture | Next visual work | Runtime/capability work | Status |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `/studio` | `prototype/src/pages/studio/MarketingRoutes.tsx` | Studio marketing | B+ | B | Standalone marketing shell, presentation toggle, polished CTAs. | Premium hero/proof blocks can go further. | Auth-aware routing and conversion analytics redaction. | Ready |
...
| `/studio/dashboard` | `prototype/src/pages/studio/DealRoutes.tsx` | Studio workspace | A- | B | Pipeline metrics, bento polish, visual coverage, sandbox read lane exists. | Minor empty/loading panel polish as real org data arrives. | Authenticated org-scoped reads and deal list persistence. | Partial |
| `/studio/deals/:dealId` | `prototype/src/pages/studio/DealRoutes.tsx` | Studio deal cockpit | A | B | Deal cockpit, bento stage posture, source evidence, trust drawer, contextual triggers. | Continue density tuning after `DealRoutes` module split. | Permissioned deal membership and evidence service reads. | Mostly done |
| `/studio/deals/:dealId/intake` | `prototype/src/pages/studio/DealRoutes.tsx` | Studio workflow | A- | C | Staged imports, upload dropzone, gate callouts, contextual handoffs. | Better live upload empty/error states once provider lane opens. | Upload service, staged extraction review, audit records. | Blocked |
| `/studio/deals/:dealId/data-review` | `prototype/src/pages/studio/DealRoutes.tsx` | Evidence workbench | A | C | DataWorkbenchShell, table/list/grid, candidate normalization, conflict copy. | Tighten reviewer side panels after adapter contract exists. | Durable extraction review, source-use policy, reviewer decisions. | Blocked |
| `/studio/deals/:dealId/comps` | `prototype/src/pages/studio/DealRoutes.tsx` | Studio comps | A | B | Institutional table, premium lock labels, source trace handoffs. | Saved views and dense filter polish if needed. | Provider rights and plan gates server-side. | Partial |
| `/studio/deals/:dealId/underwriting` | `prototype/src/pages/studio/DealRoutes.tsx` | Underwriting cockpit | A | B | Executive cockpit, AI task pulse, gates, drawer, tab transitions, contextual triggers. | Split `DealRoutes.tsx` into route modules to reduce heavy async chunk. | Versioned assumptions, gate audit, reviewer identity. | Partial |
| `/studio/deals/:dealId/underwriting/sources` | `prototype/src/pages/studio/DealRoutes.tsx` | Source trace | A | C | Source trace workbench, mock-boundary copy, review posture. | Detail panel polish after persisted source lineage exists. | Source observation lineage, reviewer decisions, audit records. | Blocked |
| `/studio/deals/:dealId/underwriting/debt` | `prototype/src/pages/studio/DealRoutes.tsx` | Debt panel | A- | C | DSCR posture, lender quote blockers, gate resolution copy. | Lender quote loading/error states after provider lane opens. | Lender quote ingestion, provider terms, reviewer gates. | Blocked |
| `/studio/deals/:dealId/scenarios` | `prototype/src/pages/studio/DealRoutes.tsx` | Scenario comparison | A | B | Scenario matrix, premium heatmap, chart, drilldown, contextual triggers. | Keyboard and mobile density audit if scenario grid expands. | Model input lineage and no AI-confidence-as-authority guarantees. | Partial |
| `/studio/deals/:dealId/versions` | `prototype/src/pages/studio/DealRoutes.tsx` | Valuation snapshots | A | C | Export eligibility, evidence snapshot refs, IC/report handoffs. | Receipt visuals once immutable snapshot contract exists. | Versioned assumptions, immutable evidence refs, receipts, approvals. | Blocked |
...
| `/studio/deals/:dealId/spatial` | `prototype/src/pages/studio/GisRoutes.tsx` | Location intelligence | A | B | GIS manifest, source rights, workbench views, layer budgets, visual snapshot. | Lazy layer state polish if approved geometry is added. | Provider/source-rights approval, lazy geometry contracts, permission filtering. | Partial |
| `/studio/reports/:dealId/builder` | `prototype/src/pages/studio/ReportRoutes.tsx` | Studio report builder | A | C | Report sections, source bundles, export blockers, preview, standalone shell. | Improve receipt-linked evidence once adapter exists. | Section approval, source-rights filtering, artifact receipts. | Blocked |
...
| `/studio/settings/white-label` | `prototype/src/pages/studio/ReportRoutes.tsx` | Studio settings | A- | C | Branding preview with evidence posture retained. | More upload/loading/error state polish after tenant storage. | Tenant branding storage and no hiding of evidence posture. | Blocked |
| `/studio/broker-os` | `prototype/src/pages/studio/OperatorRoutes.tsx` | Operator-lite | B+ | B | Sanitized job streams, agent inventory, planning context, broker shell. | Clearer operator taxonomy and diagnostic density. | Keep projection-only; raw internals stay outside public Studio. | Ready |
...
| Broker OS projection | `OperatorRoutes.tsx`, `review-assignments.ts` | Clean operator-lite panels, not public Studio chrome | Projection only, no raw queue truth | Ready |
...
| Ticket | Priority | Surface | Goal | Primary paths | Dependencies | Acceptance criteria | Status |
| --- | --- | --- | --- | --- | --- | --- | --- |
| `SOPHEX-RUN-001` | P0 | Sandbox reads | Maintain fixture/sandbox adapter posture without production claims. | `sandbox-api-client.ts`, `sandbox-api.ts`, `runtime-services.ts` | Wave 7 | Fixture fallback preserved; API mode smoke remains green. | Partial |
...
| 2026-05-26 | Wave 17 doc sync | Cross-links added from INDEX, RAPID_BUILD, MOCK_RESOLUTION, VISUAL_DESIGN_SYSTEM, WORLD_CLASS_PROTOTYPE_SPEC. | Doc integration |
