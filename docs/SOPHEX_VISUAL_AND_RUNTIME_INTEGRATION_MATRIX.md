...
| `/property/:id`       | `prototype/src/pages/PropertyPage.tsx` | Public marketplace  | A            | B             | Authority labels, map HUD, evidence drawer, loading guard, runtime shells.                                                                                                                                                                                                                           | Tune mobile map drawer density further if spatial expands.           | Server-side field resolution, permissioned evidence, spatial source-rights.   | Mostly done |
...
| `/report/:id`         | `prototype/src/pages/ReportPage.tsx`   | Public report       | A            | B             | Trust cockpit with "Public Intelligence", "Not an appraisal", "Advisory / Model-Inferred", "Review Required", source coverage/confidence/freshness cards, authority labels, deterministic fixture states, disabled export CTA, section posture strip, empty-section state, public/studio continuity. | Receipt visuals once immutable snapshot contract exists.             | Permission-filtered source bundles, section review state, reviewer receipts.  | Mostly done |
| `/export/:id`         | `prototype/src/pages/ExportPage.tsx`   | Public export gate  | A            | C             | Mock-only export readiness cockpit with deterministic fixture states, consent/source-rights/reviewer/section gates, blocker register, disabled generate action, report/review-state continuity, boundary banners, and visual snapshot posture.                                                              | Keep blocker hierarchy reviewed as more receipt states are designed. | Idempotent receipts, source-rights filtering, legal consent, governed export. | Blocked     |
| `/review/:id`         | `prototype/src/pages/ReviewPage.tsx`   | Public review queue | A            | C             | Mock-only source gap cockpit with deterministic fixture states, source gap register, local-only reviewer actions, disabled export CTA, prototype-only/no-live-approval copy, report/export/source handoffs, and visual snapshot posture. Review actions do not persist and do not clear export gates.             | Add richer receipt/timeout visuals after HITL policy decisions.      | Persisted reviewer decisions, queue assignment, audit receipts.               | Blocked     |
| `/sources/:id`        | `prototype/src/pages/ReviewPage.tsx` (sources mode) | Public source pack  | A            | C             | Mock-only source pack / citation drilldown with deterministic fixture states, citation register, source-rights blocker posture, local-only citation actions, disabled export CTA, prototype-only/no-live-source-retrieval copy, report/export/review handoffs, and visual snapshot posture. Citation actions do not persist and do not clear export gates. | Add richer citation excerpt visuals after source-rights contracts exist. | Live source retrieval, permission-filtered bundles, export unlock.              | Blocked     |
