...
| `/report/:id`         | `prototype/src/pages/ReportPage.tsx`   | Public report       | A            | B             | Trust cockpit with impossible-to-miss "Not an appraisal" posture, deterministic fixture states, source coverage/confidence/freshness cards, disabled export CTA, readiness rail, section posture strip, empty-section state, public/studio continuity. | Receipt visuals once immutable snapshot contract exists.             | Permission-filtered source bundles, section review state, reviewer receipts.  | Mostly done |
