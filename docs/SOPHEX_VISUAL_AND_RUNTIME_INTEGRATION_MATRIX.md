...
| `/review/:id`         | `prototype/src/pages/ReviewPage.tsx`   | Public review queue | A            | C             | Mock-only source gap cockpit with deterministic fixture states, source gap register, local-only reviewer actions, disabled export CTA, prototype-only/no-live-approval copy, report/export handoffs, and visual snapshot posture. Review actions do not persist and do not clear export gates.             | Add richer receipt/timeout visuals after HITL policy decisions.      | Persisted reviewer decisions, queue assignment, audit receipts.               | Blocked     |
