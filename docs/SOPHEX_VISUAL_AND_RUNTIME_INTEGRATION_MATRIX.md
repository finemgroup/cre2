...
| `/studio/dashboard` | `prototype/src/pages/studio/deals/StudioDashboardPage.tsx` | Studio workspace | A- | B | Pipeline metrics, bento polish, runtime loading shells, sandbox read lane. | Minor empty/loading panel polish as real org data arrives. | Authenticated org-scoped reads and deal list persistence. | Partial |
...
| `/studio/deals/:dealId/data-review` | `prototype/src/pages/studio/deals/StudioDataReviewPage.tsx` | Evidence workbench | A | C | DataWorkbenchShell, table/list/grid, candidate normalization, conflict copy. | Tighten reviewer side panels after adapter contract exists. | Durable extraction review, source-use policy, reviewer decisions. | Blocked |
| `/studio/deals/:dealId/comps` | `prototype/src/pages/studio/deals/StudioCompsPage.tsx` | Studio comps | A | B | Saved views, filter-empty states, runtime loading shells, premium lock labels. | Provider adapter styling when approval lane opens. | Provider rights and plan gates server-side. | Mostly done |
| `/studio/deals/:dealId/underwriting` | `prototype/src/pages/studio/deals/StudioUnderwritingPage.tsx` | Underwriting cockpit | A | B | Executive cockpit, runtime adapter shell, gates, drawer, tab transitions. | Provider adapter styling when approval lane opens. | Versioned assumptions, gate audit, reviewer identity. | Partial |
| `/studio/deals/:dealId/underwriting/sources` | `prototype/src/pages/studio/deals/StudioAssumptionSourceTracePage.tsx` | Source trace | A | C | Source trace workbench, mock-boundary copy, review posture. | Detail panel polish after persisted source lineage exists. | Source observation lineage, reviewer decisions, audit records. | Blocked |
| `/studio/deals/:dealId/underwriting/debt` | `prototype/src/pages/studio/deals/StudioDebtPanelPage.tsx` | Debt panel | A- | C | DSCR posture, lender quote blockers, gate resolution copy. | Lender quote loading/error states after provider lane opens. | Lender quote ingestion, provider terms, reviewer gates. | Blocked |
| `/studio/deals/:dealId/scenarios` | `prototype/src/pages/studio/deals/StudioScenarioComparisonPage.tsx` | Scenario comparison | A | B | Scenario matrix, premium heatmap, chart, drilldown, keyboard hint copy, contextual triggers. | Mobile density audit if scenario grid expands further. | Model input lineage and no AI-confidence-as-authority guarantees. | Mostly done |
| `/studio/deals/:dealId/versions` | `prototype/src/pages/studio/deals/StudioValuationVersionTimelinePage.tsx` | Valuation snapshots | A | C | Export eligibility, evidence snapshot refs, IC/report handoffs. | Receipt visuals once immutable snapshot contract exists. | Versioned assumptions, immutable evidence refs, receipts, approvals. | Blocked |
...
| `SOPHEX-CRE-003` | P1 | Comps adapter | Prepare comps surface for approved sandbox/core analysis route with fixture fallback. | `CompsPage.tsx`, `deals/StudioCompsPage.tsx`, runtime adapters | Approval for adapter lane | Styled loading/failure/empty/result states, provider rights visible. | Partial |
| `SOPHEX-CRE-004` | P1 | Underwriting adapter | Prepare underwriting cockpit for approved sandbox/core output adapter. | `deals/StudioUnderwritingPage.tsx`, `lib/underwriting/*`, runtime adapters | Approval for adapter lane | Assumptions, metrics, gates map to shared authority vocabulary. | Partial |
