...
| --------------------- | -------------------------------------- | ------------------- | ------------ | ------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------- | ----------------------------------------------------------------------------- | ----------- |
| `/`                   | `prototype/src/pages/LandingPage.tsx`  | Public marketplace  | A            | B             | Runtime landing guide, adapter-backed proof strip, source-trust panel, search shells.                                                                                                                                                                                                                | Continue conversion hierarchy if GTM becomes active.                 | Keep fixture/sandbox public search until data-source contracts clear.         | Mostly done |
| `/property/:id`       | `prototype/src/pages/PropertyPage.tsx` | Public marketplace  | A            | B             | Authority labels, map HUD, evidence drawer, loading guard, runtime shells, guided demo rail with URL-only fixture state continuity.                                                                                                                                                                                                                           | Tune mobile map drawer density further if spatial expands.           | Server-side field resolution, permissioned evidence, spatial source-rights.   | Mostly done |
| `/property/:id/comps` | `prototype/src/pages/CompsPage.tsx`    | Public marketplace  | A            | B             | Saved comp views, filter-empty states, dense table, map/list context, blocked/private labels, guided demo rail with report handoff preserving `?state=`.                                                                                                                                                                                                        | Provider adapter styling when approval lane opens.                   | Provider rights, comp source restrictions, private-value isolation.           | Mostly done |
...
| `/upload`             | `prototype/src/pages/UploadPage.tsx`   | Public contribution | A-           | B             | Runtime upload guide, contribution proof strip, consent copy, candidate-evidence result.                                                                                                                                                                                                             | Live file scanning/storage once provider lane opens.                 | File scanning, storage, source metadata, extraction candidates, review gates. | Mostly done |
| `/report/:id`         | `prototype/src/pages/ReportPage.tsx`   | Public report       | A            | B             | Trust cockpit with "Public Intelligence", "Not an appraisal", "Advisory / Model-Inferred", "Review Required", source coverage/confidence/freshness cards, authority labels, deterministic fixture states, disabled export CTA, guided demo rail, section posture strip, empty-section state, public/studio continuity. | Receipt visuals once immutable snapshot contract exists.             | Permission-filtered source bundles, section review state, reviewer receipts.  | Mostly done |
| `/export/:id`         | `prototype/src/pages/ExportPage.tsx`   | Public export gate  | A            | C             | Mock-only export readiness cockpit with deterministic fixture states, consent/source-rights/reviewer/section gates, blocker register, disabled generate action, guided demo rail, report/review-state continuity, boundary banners, and visual snapshot posture.                                                              | Keep blocker hierarchy reviewed as more receipt states are designed. | Idempotent receipts, source-rights filtering, legal consent, governed export. | Blocked     |
| `/review/:id`         | `prototype/src/pages/ReviewPage.tsx`   | Public review queue | A            | C             | Mock-only source gap cockpit with deterministic fixture states, source gap register, local-only reviewer actions, disabled export CTA, guided demo rail, prototype-only/no-live-approval copy, report/export/source handoffs, and visual snapshot posture. Review actions do not persist and do not clear export gates.             | Add richer receipt/timeout visuals after HITL policy decisions.      | Persisted reviewer decisions, queue assignment, audit receipts.               | Blocked     |
| `/sources/:id`        | `prototype/src/pages/ReviewPage.tsx` (sources mode) | Public source pack  | A            | C             | Mock-only source pack / citation drilldown with deterministic fixture states, citation register, source-rights blocker posture, local-only citation actions, disabled export CTA, guided demo rail, prototype-only/no-live-source-retrieval copy, report/export/review handoffs, and visual snapshot posture. Citation actions do not persist and do not clear export gates. | Add richer citation excerpt visuals after source-rights contracts exist. | Live source retrieval, permission-filtered bundles, export unlock.              | Blocked     |
...
| `/studio/onboarding`                         | `prototype/src/pages/studio/MarketingRoutes.tsx`                          | Studio onboarding     | A-           | C             | Step wizard, runtime onboarding adapter shell, proof strip, segmented transitions, presentation shell. | More branded side preview and selected-template polish.           | Account creation, org creation, billing, terms acceptance.                      | Blocked     |
| `/studio/dashboard`                          | `prototype/src/pages/studio/deals/StudioDashboardPage.tsx`                | Studio workspace      | A-           | B             | Pipeline metrics from adapter, workspace proof strip, runtime loading shells, empty pipeline state.    | Minor polish as real org data arrives.                            | Authenticated org-scoped reads and deal list persistence.                       | Mostly done |
...
| `/studio/deals/:dealId/intake`               | `prototype/src/pages/studio/deals/StudioDealIntakePage.tsx`               | Studio workflow       | A-           | B             | Runtime adapter shell, intake proof strip, staged import preview, data-review handoffs.                | Live upload empty/error states once provider lane opens.          | Upload service, staged extraction review, audit records.                        | Mostly done |
...
| `/studio/deals/:dealId/underwriting`         | `prototype/src/pages/studio/deals/StudioUnderwritingPage.tsx`             | Underwriting cockpit  | A            | B             | Executive cockpit, runtime adapter shell, gate proof strip, evidence cross-links.                      | Provider adapter styling when approval lane opens.                | Versioned assumptions, gate audit, reviewer identity.                           | Mostly done |
...
| `/studio/settings/billing`                   | `prototype/src/pages/studio/MarketingRoutes.tsx`                          | Studio settings       | A-           | B             | Runtime billing adapter shell, plan proof strip, feature comparison, FAQ.                              | Live checkout once billing provider opens.                        | Billing provider and plan entitlements.                                         | Mostly done |
...
| `/studio/broker-os`                          | `prototype/src/pages/studio/OperatorRoutes.tsx`                           | Operator-lite         | A-           | B             | Runtime adapter shell, projection proof strip, sanitized job streams, review queue preview.            | Diagnostic density if operator workflows expand.                  | Keep projection-only; raw internals stay outside public Studio.                 | Mostly done |
...
| 2026-05-26 | Wave 28 intake closeout               | Deal intake runtime adapter, underwriting gate proof strip, evidence workflow cross-links.                                                                                                  | 328 unit tests, visual snapshots                                              |
| 2026-05-26 | Wave 29 public upload closeout        | Public upload guide adapter, broker OS runtime shell, source trace handoffs, dashboard proof strip.                                                                                         | 330 unit tests, visual snapshots                                              |
| 2026-05-26 | Wave 30 marketplace settings closeout | Landing guide adapter, property loading guard, billing plan runtime shell.                                                                                                                  | 332 unit tests, visual snapshots                                              |
| 2026-05-26 | Wave 31 governance prep closeout      | Export gate + onboarding runtime adapters, proof strips, report/studio cross-links.                                                                                                         | 336 unit tests, visual snapshots                                              |
...
| 2026-05-27 | `SOPHEX-REPORT-002`                   | Public report trust copy and fixture-state cockpit added on stacked branch over PR #4. Export remains disabled/gated and prototype-only copy remains visible.                               | `ReportPage.tsx`, `smoke.spec.ts`, mock registry, closeout                    |
| 2026-05-27 | `SOPHEX-OVERNIGHT-REPORT-MATRIX-001`  | Overnight stacked lane tightened required public report trust copy, fixture-state coverage, matrix/mock-registry tracking, and gated export assertions.                                     | PR #5, `ReportPage.tsx`, `smoke.spec.ts`, mock registry, closeout             |
| 2026-05-29 | `SOPHEX-EXPORT-GATE-001`              | Public export gate now mirrors deterministic report fixture states with consent, source-rights, reviewer approval, section-readiness blockers, and disabled generation in all states.       | `ExportPage.tsx`, `ExportGovernanceModal.tsx`, e2e/unit coverage, closeout    |
| 2026-05-29 | `SOPHEX-REVIEW-QUEUE-001`             | Public review queue/source gap cockpit added after export gate with fixture-state continuity, source gap register, local-only reviewer actions, disabled export, and route handoffs.        | `ReviewPage.tsx`, `public-review-queue.ts`, e2e/unit/visual coverage, closeout |
| 2026-05-29 | `SOPHEX-SOURCE-EVIDENCE-002`          | Public source pack/citation drilldown cockpit added after review queue with fixture-state continuity, citation register, source-rights blocker posture, local-only citation actions, disabled export, and route handoffs. | `ReviewPage.tsx`, `public-export-fixtures.ts`, e2e/unit/visual coverage, closeout |
| 2026-05-29 | `SOPHEX-DEMO-STORY-003`               | Mock-only guided demo rail added across property, comps, report, export gate, review queue, and source pack with URL/query-state fixture continuity, narrative copy per state, prototype-only posture, and disabled export preserved. | `GuidedDemoRail.tsx`, `public-export-fixtures.ts`, `PublicShell.tsx`, e2e coverage, closeout |
