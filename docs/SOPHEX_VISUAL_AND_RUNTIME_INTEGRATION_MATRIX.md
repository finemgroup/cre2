...
| `/studio/reports/:dealId/builder` | `prototype/src/pages/studio/ReportRoutes.tsx` | Studio report builder | A | C | Report sections, runtime adapter shell, export blockers, empty-section state. | Improve receipt-linked evidence once adapter exists. | Section approval, source-rights filtering, artifact receipts. | Partial |
