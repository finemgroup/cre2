...
| `/studio/deals/:dealId/comps` | `prototype/src/pages/studio/deals/StudioCompsPage.tsx` | Studio comps | A | B | Saved views, filter-empty states, institutional table, premium lock labels, source trace handoffs. | Provider adapter styling when approval lane opens. | Provider rights and plan gates server-side. | Mostly done |
| `/studio/deals/:dealId/underwriting` | `prototype/src/pages/studio/deals/StudioUnderwritingPage.tsx` | Underwriting cockpit | A | B | Executive cockpit, AI task pulse, gates, drawer, tab transitions, contextual triggers. | Continue import cleanup on remaining deal route modules. | Versioned assumptions, gate audit, reviewer identity. | Partial |
